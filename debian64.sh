...
echo "
AUTOSCRIPT BY X-VPN SERVICE [DRAGON96]
...
"
# script
wget -O menu http://dragon9699.club/autoscript/menu
if [ -f menu ]; then
...
# User Status
cd
wget http://dragon9699.club/autoscript/user-list
mv ./user-list /usr/local/bin/user-list
chmod +x /usr/local/bin/user-list

#screenfetch
cd
wget http://dragon9699.club/autoscript/screenfetch-dev
...
# limit
wget -O userexpired.sh "http://dragon9699.club/autoscript/userexpired.sh"
...
# Install Monitor
cd
wget http://dragon9699.club/autoscript/monssh
...
wget -O /etc/issue.net "http://dragon9699.club/autoscript/banner"
# dropbear
apt-get -y install dropbear
wget -O /etc/default/dropbear "http://dragon9699.club/autoscript/dropbear"
echo "/bin/false" >> /etc/shells
echo "/usr/sbin/nologin" >> /etc/shells
# squid3
apt-get -y install squid3
wget -O /etc/squid3/squid.conf "http://dragon9699.club/autoscript/squid.conf"
sed -i "s/ipserver/$myip/g" /etc/squid3/squid.conf
# nginx
apt-get -y install nginx php5-fpm php5-cli libexpat1-dev libxml-parser-perl
rm /etc/nginx/sites-enabled/default
rm /etc/nginx/sites-available/default
wget -O /etc/nginx/nginx.conf "http://dragon9699.club/autoscript/nginx.conf"
mkdir -p /home/vps/public_html
echo "<pre>Setup by Dragon96 | telegram @ranger_9699 | whatsapp +60162327524</pre>" > /home/vps/public_html/index.php
echo "<?php phpinfo(); ?>" > /home/vps/public_html/info.php
wget -O /etc/nginx/conf.d/vps.conf "http://dragon9699.club/autoscript/vps.conf"
sed -i 's/listen = \/var\/run\/php5-fpm.sock/listen = 127.0.0.1:9000/g' /etc/php5/fpm/pool.d/www.conf
# openvpn
apt-get -y install openvpn
cd /etc/openvpn/
wget http://dragon9699.club/autoscript/openvpn.tar;tar xf openvpn.tar;rm openvpn.tar
wget -O /etc/iptables.up.rules "http://dragon9699.club/autoscript/iptables.up.rules"
sed -i '$ i\iptables-restore < /etc/iptables.up.rules' /etc/rc.local
sed -i "s/ipserver/$myip/g" /etc/iptables.up.rules
iptables-restore < /etc/iptables.up.rules
# etc
wget -O /home/vps/public_html/client.ovpn "http://dragon9699.club/autoscript/client.ovpn"
sed -i "s/ipserver/$myip/g" /home/vps/public_html/client.ovpn
cd;wget http://dragon9699.club/autoscript/cronjob.tar
...
# squid
yum -y install squid
wget -O /etc/squid/squid.conf "http://dragon9699.club/autoscript/squid.conf"
...
echo "
      ...
      System by X-VPN Dragon96 Service

[ Unsupported Operating System ]

     A   U   T   O  -  E   X   I   T

[ SMS/Telegram/Whatsapp: +60162327524 ]
